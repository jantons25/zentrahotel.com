...
export async function RoomCard({ room, showAvailabilityCta = false }: RoomCardProps) {
  const t = await getTranslations("rooms.card");
  ...
  return (
    <Card className="group overflow-hidden border-border py-0 shadow-card transition-shadow duration-(--duration-normal) hover:shadow-card-hover">
      ...
      <CardContent className="space-y-1.5 p-5 pt-4 pb-6">
        <h3 className="text-lg font-bold text-primary">{pick(room.name, locale)}</h3>
        <p className="text-sm text-muted-foreground">{pick(room.capacity, locale)}</p>
        <p className="text-sm text-muted-foreground">{pick(room.detail, locale)}</p>
        {showAvailabilityCta ? (
          <a
